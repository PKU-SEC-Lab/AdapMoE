...
class Counter():
    ...
    def update(self, num):
        if isinstance(num, torch.Tensor):
            if num.dim() == 0:
                self.nums.append(num.item())
                self.num = self.num * (self.count / (self.count + 1)) + num.item() / (self.count + 1)
                self.count += 1
            else:
                self.nums += num.tolist()
                self.num = self.num * (self.count / (self.count + num.size(0))) + num.sum().item() / (self.count + num.size(0))
                self.count += num.size(0)
        else:
            self.nums.append(num)
            self.num = self.num * (self.count / (self.count + 1)) + num / (self.count + 1)
            self.count += 1
...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        #modified
        self.threshold = 0
        self.top1_ratio = Counter()
    
    def compare_experts(self, hidden_states:torch.Tensor):
        # caculate the logits for all the experts
        outputs =[]
        for i in range(self.num_experts):
            outputs.append(self.experts[i](hidden_states))
        #compare the norm
        for i in range(self.num_experts):
            for j in range(i,self.num_experts):
                if i != j:
                    norm = torch.norm(outputs[i] - outputs[j], dim=-1).mean()
                    self.norm_between_experts[i][j].update(norm)
                    self.norm_between_experts[j][i].update(norm)
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        #modified
        if self.threshold > 0:
            routing_weights[:, 1] = torch.where(routing_weights[:, 1] < self.threshold, torch.zeros(routing_weights.size()[0], device = routing_weights.device), routing_weights[:, 1])
            routing_weights /= routing_weights.sum(dim=-1, keepdim=True)
            # 统计routing_weights[:, 1] < self.threshold的比例
            ratio = (routing_weights[:, 1] < self.threshold).sum().item()/routing_weights.size()[0]
            self.top1_ratio.update(ratio)
...
class MixtralModel(MixtralPreTrainedModel):
    ...
    def __init__(self, config: MixtralConfig):
        ...
        for idx, layer in enumerate(self.layers):
            layer.block_sparse_moe.layer_id = idx
